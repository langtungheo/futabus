...
export default function Buytickets() {
	...
	const columns = [
		{
			...
			render: (text, record, index) => {
				return (
					<span key={Date.now()} className="text-lg" style={{ color: '#ef5222' }}>
						{text}
					</span>
				);
			},
	...
	const handleClick = () => {
		openNotificationWithIcon(
			'warning',
			<span className="text-red-500">Xin lỗi</span>,
			'Chức năng đang trong qúa trình xây dựng !'
		);
	};
			...
			<div className="row overflow-hidden px-2">
					...
					<div className="mt-4">
						<div className="">
							<span className="text-xl">
								Tìm thấy {trips.length} chuyến đi từ
								<span style={{ color: '#ef5222' }}> {trips[0].OriginName} </span>
								đến
								<span style={{ color: '#ef5222' }}> {trips[0].DestName}</span>
							</span>
						</div>
				...
				<div className="mt-10 mb-5  md:my-5 tab-ticket">
					<Tabs defaultActiveKey="1" centered>
						<TabPane
							tab={<span className="uppercase font-bold">thông tin chung</span>}
							key="1"
						>
							{trips ? (
								<div className="">
									<p
										...
										style={{ color: '#ef5222' }}
									>
										{trips[0].OriginName} ⇒ {trips[0].DestName}
									</p>
									<p className="mb-0">
										Khoảng cách:{' '}
										<span className="font-bold text-black">
											{Math.floor(minDisc / 1000)} km ~{' '}
											{Math.floor(maxDisc / 1000)} km
										</span>
									</p>
									<p className="mb-0">
										Thời gian:{' '}
										<span className="font-bold text-black">
											{Math.floor(parseInt(trips[0].Duration) / 3600)} h
										</span>
									</p>
									<p className="mb-0">
										* Lưu ý: thời gian đón và trung chuyển xe có thể thay đổi
										phụ thuộc vào mật độ giao thông tại từng thời điểm. Kính
								...
								<div className="">
									...
									<p className="mb-0">
										Khoảng cách:{' '}
										<span className="font-bold text-black">0 km</span>
									</p>
									<p className="mb-0">
										Thời gian: <span className="font-bold text-black">0 h</span>
									</p>
									<p className="mb-0">
										* Lưu ý: thời gian đón và trung chuyển xe có thể thay đổi
										phụ thuộc vào mật độ giao thông tại từng thời điểm. Kính
						...
						<TabPane
							tab={<span className="uppercase font-bold">điểm đón xe</span>}
							key="2"
						>
							Đang cập nhật...
						</TabPane>
						<TabPane
							tab={<span className="uppercase font-bold">điểm trung chuyển</span>}
							key="3"
						>
							Đang cập nhật...
						</TabPane>
